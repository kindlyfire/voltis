...
class _FoundItem(TypedDict):
    type: Literal["file", "directory"]
    path: anyio.Path
    children: list["_FoundItem"] | None
...
class ScannerBase(ABC):
    ...
    @abstractmethod
    async def scan_items(self, item: _FoundItem) -> list[Content]:
        """
    ...
    async def find_items(self) -> _FoundItem:
        """
        Walk all folders in self.ds.path recursively up to depth 5, returning a
        tree structure.

        Returns:
            A _FoundItem representing the root folder with all its children.
        """

        async def _inner(path: anyio.Path, depth: int) -> _FoundItem | None:
            if depth > 5:
                return None

            children: list[_FoundItem] = []
            ...
            async for item in path.iterdir():
                if await item.is_dir():
                    ...
                    children.append(_FoundItem(type="file", path=item, children=None))

            return _FoundItem(type="directory", path=path, children=children if children else None)
